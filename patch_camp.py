# -*- coding: utf-8 -*-
"""Patch camp-vacances.html:
   1. Fix hero image path (leading slash → relative)
   2. Add CSS for card photo zoom
   3. Programme A card: add cover photo + remove emoji
   4. Programme B card: add cover photo + remove emoji
"""
import sys, io
...
c = c.replace(
    "style=\"--hero-image: url('/assets/img/camp-vacances-hero.jpg');",
    "style=\"--hero-image: url('assets/img/camp-vacances-hero.jpg');"
)

# ─── 2. ADD ANIMATION CSS ──────────────────────────────────────────────────────
c = c.replace(
    '   .sticky-cta { backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }\n  </style>',
    '''   .sticky-cta { backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
   /* card photo zoom on hover */
   .prog-card-photo { overflow: hidden; }
   .prog-card-photo img { transition: transform .6s cubic-bezier(.25,.46,.45,.94); }
   .card-hover:hover .prog-card-photo img { transform: scale(1.06); }
   /* POPULAIRE badge pulse */
   @keyframes badgePulse { 0%,100%{box-shadow:0 0 0 0 rgba(245,158,11,.5)} 50%{box-shadow:0 0 0 8px rgba(245,158,11,0)} }
   .badge-pulse { animation: badgePulse 2.4s ease-in-out infinite; }
  </style>'''
)

# ─── 3. PROGRAMME A CARD (full replacement) ────────────────────────────────────
OLD_A = '''          <!-- PROGRAMME A -->
...
NEW_A = '''          <!-- PROGRAMME A -->
          <div class="rounded-3xl overflow-hidden shadow-lg card-hover reveal reveal-left" style="border-top:4px solid #1e7a9a; background:linear-gradient(155deg,#fff 0%,#f0f9fc 100%);">
            ...
            <div class="prog-card-photo relative h-52">
              <img src="assets/img/camp-prog-a.jpg" alt="Programme A — Self-défense Cité des Magistrats" class="w-full h-full object-cover" loading="lazy">
...
# ─── 4. PROGRAMME B CARD (full replacement) ────────────────────────────────────
OLD_B = '''          <!-- PROGRAMME B -->
...
# ─── APPLY ────────────────────────────────────────────────────────────────────
patches = [
    ('Hero image path', "url('/assets/img/camp-vacances-hero.jpg')", "url('assets/img/camp-vacances-hero.jpg')"),
    ('CSS animation', '   .sticky-cta { backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }\n  </style>',
     '''   .sticky-cta { backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); }
   .prog-card-photo { overflow: hidden; }
   .prog-card-photo img { transition: transform .6s cubic-bezier(.25,.46,.45,.94); }
   .card-hover:hover .prog-card-photo img { transform: scale(1.06); }
...
# Cards
for label, old, new in [('Programme A card', OLD_A, NEW_A), ('Programme B card', OLD_B, NEW_B)]:
    if old in c:
        c = c.replace(old, new, 1)
        ok += 1
        print(f'OK: {label}')
...
if ok == 5:
    ...
    print(f'\n{ok}/5 patches OK — file NOT saved.')
